...
/**
 * To connect Mavericks ViewModel creation with Hilt's dependency injection, add the
 * following Factory and companion object to your MavericksViewModel.
 *
 * Example:
 *
 ...
 *   }
 *
 *   companion object : MavericksViewModelFactory<MyViewModel, MyState> by
...
class HiltMavericksViewModelFactory<VM : MavericksViewModel<S>, S : MavericksState>(
    ...
    override fun create(viewModelContext: ViewModelContext, state: S): VM {
        // We want to create the ViewModelComponent. In order to do that, we need to get its
...
/**
 * Hilt's ViewModelComponent's parent is ActivityRetainedComponent but there is no easy way to
 * access it. SingletonComponent should be sufficient
 * because the ViewModel that gets created is the only object with a reference to the created
 * component so the lifecycle of it will
 * still be correct.
 */
